...
<xsl:stylesheet version="1.0"
  ...
  <xsl:key name="label_branches" match="//idx:*[@label]|//lv:*[@label]" use="@label"/>
  <xsl:key name="statement_branches" match="//idx:branch[@md5]|//idx:statement[@md5]|//idx:substatement[@md5]|//idx:figure[@md5]" use="@md5"/>
  <xsl:key name="ref_branches" match="//idx:refby[@md5]|//idx:ref[@md5]" use="concat(@file_md5, '-', @slide, '-', @md5)"/>

  <xsl:template match="/">
      <document>
          <index>
              <xsl:apply-templates select="//idx:keyword[generate-id() = generate-id(key('keyword_branches', @keyword)[1])]|//idx:branch[generate-id() = generate-id(key('keyword_branches', @keyword)[1])]|//lv:keyword[generate-id() = generate-id(key('keyword_branches', @keyword)[1])]|//idx:branch[generate-id() = generate-id(key('keyword_branches', @keyword)[1])]"/>
              <!-- <xsl:apply-templates select="//*[@label and (generate-id() = generate-id(key('label_branches', @label)[1]))]"/> -->
              <xsl:apply-templates select="//idx:branch[@md5 and (generate-id() = generate-id(key('statement_branches', @md5)[1]))]|//idx:statement[@md5 and (generate-id() = generate-id(key('statement_branches', @md5)[1]))]|//idx:substatement[@md5 and (generate-id() = generate-id(key('statement_branches', @md5)[1]))]|//idx:figure[@md5 and (generate-id() = generate-id(key('statement_branches', @md5)[1]))]"/>
  ...
  <xsl:template match="idx:branch[@keyword]|idx:keyword|lv:keyword">
	...
	<xsl:value-of select="@keyword"/>
      ...
      <xsl:for-each select="//idx:branch[@keyword=$keyword and (generate-id() = generate-id(key('keyword_branches_slide', concat(@file_md5, '-', @slide, '-', @keyword))[1]))]|//idx:keyword[@keyword=$keyword and (generate-id() = generate-id(key('keyword_branches_slide', concat(@file_md5, '-', @slide, '-', @keyword))[1]))]|//lv:keyword[@keyword=$keyword and (generate-id() = generate-id(key('keyword_branches_slide', concat(@file_md5, '-', @slide, '-', @keyword))[1]))]">
      <!-- <xsl:for-each select="key('keyword_branches', @keyword)"> -->
	<branch>
	  <xsl:copy-of select="@*"/>
  ...
  <xsl:template match="idx:branch[@md5]|idx:statement[@md5]|idx:substatement[@md5]|idx:figure[@md5]">
      <entry>
              ...
              <branch>
                  ...
                  <xsl:if test="@of">
                      ...
                      <xsl:attribute name="ofmd5">
                          <xsl:value-of select="(//idx:*[@label=$of]|//idx:*[@md5=$of])/@md5"/>
                      </xsl:attribute>
                  </xsl:if>
